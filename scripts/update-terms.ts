...
type WebsocTerm = { shortName: string; longName: string };
type WebsocSchool = {
  departments: { courses: { sections: unknown[] }[] }[];
};
...
async function updateTerms() {
    try {
        ...
        const countChanged = Math.abs(currentCount - deployedData.sectionCount) > 0;
